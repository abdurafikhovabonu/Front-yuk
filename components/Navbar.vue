      ...
      <div class="flex justify-between items-center h-20">
        ...
        <NuxtLink to="/" class="flex items-center space-x-3 group">
          <div class="relative">
            ...
            <div class="absolute -top-2 -right-2 bg-yellow-400 text-blue-900 text-xs font-bold rounded-full px-1.5 py-0.5 uppercase">
              {{ locale }}
            </div>
          </div>
          <div class="flex flex-col">
            ...
            <span class="text-xs text-gray-500 -mt-1">{{ t('nav.tagline') }}</span>
          </div>
        </NuxtLink>
            ...
            >
              <div class="flex items-center space-x-2">
                ...
                <div class="hidden lg:block text-left">
                  ...
                  <p class="text-xs text-gray-500">{{ roleLabel(user.role) }}</p>
                </div>
            ...
            <transition name="fade">
              ...
              >
                ...
                >
                  ...
                  {{ user.role === 'client' ? t('nav.myOrders') : t('nav.myProposals') }}
                </NuxtLink>
...
const roleLabel = (r) =>
  ({ client: t('roles.client'), driver: t('roles.driver'), admin: t('roles.admin') }[r] || r)

const menuItems = computed(() => {
  void locale.value
  const commonItems = [{ name: t('nav.home'), path: '/' }]
  ...
  if (user.value.role === 'admin') {
    return [
      ...commonItems,
      { name: t('nav.adminPanel'), path: '/admin' },
      { name: t('nav.adminUsers'), path: '/admin/users' },
      { name: t('nav.adminOrders'), path: '/admin/orders' },
      { name: t('nav.adminDrivers'), path: '/admin/drivers' },
      { name: t('admin.news'), path: '/admin/news' }
    ]
  }
  ...
  if (user.value.role === 'client') {
    return [
      ...commonItems,
      { name: t('nav.createLoad'), path: '/create-order' },
      { name: t('nav.orders'), path: '/orders' },
      { name: t('nav.incomingProposals'), path: '/incoming-proposals' },
      { name: t('nav.drivers'), path: '/drivers' },
  ...
  if (user.value.role === 'driver') {
    return [
      ...commonItems,
      { name: t('nav.loads'), path: '/loads' },
      { name: t('nav.myProposals'), path: '/my-proposals' },
      { name: t('nav.drivers'), path: '/drivers' },
      { name: t('nav.news'), path: '/news' }
    ]
  }
